workspace "Chatbot IA Local" "Asistente conversacional SAPUI5 con IA local, motor RAG y consultas OData" {
    ...
    model {
        proveedor = person "Proveedor" "Usuario que consulta datos de negocio (facturas, pedidos, clientes) y documentacion corporativa (FAQ, manuales, glosario)"
        ...
        chatbot = softwareSystem "SAPUI5 Chat" "Aplicacion conversacional para consulta de datos de negocio y documentacion corporativa via lenguaje natural" {
            ...
            backend = container "Backend" "API REST que orquesta clasificacion de intencion via LLM, consultas OData validadas, busqueda RAG y generacion de respuesta natural" "Node.js 22 + Express 4" {
                ...
                documentEngine = component "Document Engine" "db/engine.js - Busqueda en cascada: FAQ, Glosario (ILIKE), Chunks (FTS espanol)" "JavaScript"
        ...
        postgresql = softwareSystem "PostgreSQL 18" "Base de datos chatbot_rag con FTS espanol. Tablas: documents, document_chunks, faq, glossary" {
            tags "External"
        }

        northwind = softwareSystem "Northwind OData" "API OData externa services.odata.org (Orders, Customers, Order_Details). Solo GET validado" {
            tags "External"
        }
        ...
        backend -> postgresql "Busca documentacion (FAQ, Glosario, Chunks FTS)"
